...
def corregir_nombre(nombre_nuevo, lista_existente):
    # Si la lista está vacía, devolvemos el nombre tal cual
    if not lista_existente:
        return nombre_nuevo
    # Busca parecidos (umbral 0.6 = 60% de similitud)
...
# --- 2. TÍTULO Y DASHBOARD (FIJO ARRIBA) ---
...
with col_izq:
    ...
    with tab1:
        if not df_stock.empty:
            ...
            # Usamos selectbox para que en la venta NO pueda escribir mal
            ...
            # LÍNEA CORREGIDA (Sin los $$):
                ...
                if st.form_submit_button("REGISTRAR VENTA"):
                    if kgs_vender > 0:
                        ...
                        # B. Registramos la plata que entró
    ...
    with tab2:
        with st.form("form_stock", clear_on_submit=True):
            ...
            if st.form_submit_button("Cargar a Heladera"):
                if prod_input:
                    # NORMALIZACIÓN Y CORRECCIÓN FUZZY
                    ...
                    conn.commit(); conn.close()
                    st.success(f"Cargado como: {nombre_final}")
                    st.rerun()
...
with col_der:
    ...
    st.markdown("---")
    # BOTÓN PARA DESCARGAR EXCEL (Idea final)
